...
/***/ function(module, exports, __webpack_require__) {
	...
	function getBlockedAST(program) {
	    var tab = null; // as array for easy processing
	    var i = 0;
	    return __assign({}, getBlocks(0), { newLines: program.newLines, type: "Program" });
	    function getBlocks(tabLevel) {
	        var newBlock = {
	            type: "Block",
	            code: []
	        };
	        for (; i < program.code.length; i++) {
	            var line = program.code[i];
	            if (line.tabs === "" && tab === null) {
	                ...
	                if (line.type === "Blank") {
	                ...
	                else if (!tabArray.every(function (x) { return _.isEqual(tab, x); })) {
	                    ...
	                    if (tabArray.length === tabLevel) {
	                        ...
	                        throw error("Parse Error: Cannot have a gap of more than 1 between succesive indentations\n" +
	                            ("Found a gap of " + Math.abs(tabArray.length - tabLevel)));
	                    }
	                }
	            }
	        }
	...
	exports.getBlockedAST = getBlockedAST;
	var macros = [{
	        id: "on",
	        map: function (args, compile, innerCode) {
	...
	function compileProgram(program) {
	    ...
	    var newInnerBlock = innerBlockLines.map(function (x) { return "    " + x; }).join("\n");
	    return "$(function(){\n" + newInnerBlock + "\n})";
	}
	exports.compileProgram = compileProgram;
	function compileBlock(block) {
	    var code = [];
	    function addStyle(line, generatedCode) {
	        var commentRegion = "";
	        if (line.lineComment) {
	    ...
	    for (var i = 0; i < block.code.length; i++) {
	        ...
	        if (_line.type === "Macro") {
	            (function (line) {
	                var potentialMacro = _.find(macros, function (macro) { return macro.id == line.id.text; });
	                ...
	                var macroOutputLines = potentialMacro.map(line.args, compileExpression, compileBlock(macroBlock)).split("\n");
	                code.push([
	                    addStyle(line, macroOutputLines[0])
	                ].concat(macroOutputLines.slice(1)).join("\n"));
	                i++;
	            })(_line);
	        }
	...
	function compileSelectorStatement(stm) {
	...
	module.exports = (function() {
	  ...
	  function peg$parse(input) {
	    var options = arguments.length > 1 ? arguments[1] : {},
	        ...
	        peg$c1 = function(tabs, statement, lineComment) {
	        ...
	        peg$c3 = function(selector, func, args) { 
	            return {
	        ...
	        peg$c61 = function(whitespace) {
	                return text()
	            },
	        peg$c62 = { type: "other", description: "whitespace with a new line" },
	        peg$c63 = function() {
	                return text()
	            },
	    ...
	    function peg$parseAnythingSameLine() {
	      var s0, s1, s2, s3;
